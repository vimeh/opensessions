...
- Fast switching: `j`/`k` and arrows switch as soon as a session row is highlighted (no `Enter`; rapid scanning is coalesced server-side so the latest row wins), wrap around at the top/bottom of the list, plus `Tab`, `1`-`9`, session reordering, hide/restore, creation, and kill actions.
