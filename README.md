...
- Fast switching: `j`/`k` and arrows switch instantly on a discrete press (no `Enter`) and settle on the resting row during key-repeat, wrap around at the top/bottom of the list, plus `Tab`, `1`-`9`, session reordering, hide/restore, creation, and kill actions.
