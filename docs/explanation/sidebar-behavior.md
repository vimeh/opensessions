...
The sidebar session list has one durable local active row: this tmux client's confirmed active session. Keyboard navigation is highlight-driven with an adaptive debounce: a discrete `j`/`k`/arrow press onto a concrete session row requests the switch immediately — no `Enter` required and no added latency — while movement arriving within the debounce window of the previous movement (key repeat, rapid tapping) arms a short debounce (`HIGHLIGHT_SWITCH_DEBOUNCE_MS`, 150ms) and switches once the highlight rests. The debounce exists because each tmux switch costs ~40ms plus hook fan-out and per-client broadcasts, so key-repeat browsing must not fire a switch per crossed row; only the row the highlight rests on switches. Navigation wraps at the list edges: `k`/Up from the top row lands on the bottom row and `j`/Down from the bottom row lands back on the top row, with the same rules applying to the wrapped-onto row. The highlighted row stays visible as the pending switch target until `YourSession`/pane identity confirms the new context; it must not snap back to the old active row for an intermediate frame. Landing on the already-confirmed session with no switch in flight sends nothing and disarms any armed switch, and re-highlighting the current in-flight switch target sends nothing; a rapid reversed browse settles on the final highlighted row. `Enter` on a concrete session row commits immediately and supersedes the armed debounce, as do `Tab`/`Shift-Tab`, digit keys, and mouse clicks. Mouse hover stays visual-only and never switches. In all cases, the durable active row stays on the confirmed active session until confirmation.

Worktree group headers are the one browse-only focus target. `j`/`k`/arrow navigation can land on them without switching anything — moving onto a header also disarms any armed highlight switch — and `Enter` toggles collapse/expand. Once the user highlights a concrete child session inside an expanded worktree group and the highlight rests, the switch fires and pending focus moves from the group header to that child session row; the group header must not remain focused in the destination session.
