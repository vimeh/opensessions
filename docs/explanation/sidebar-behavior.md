...
The sidebar session list has one durable local active row: this tmux client's confirmed active session. Keyboard navigation is highlight-driven: when `j`/`k`/arrow movement lands on a concrete session row, the sidebar arms a short debounce (`HIGHLIGHT_SWITCH_DEBOUNCE_MS`, 150ms) and requests a switch to that session once the highlight rests — no `Enter` required. Navigation wraps at the list edges: `k`/Up from the top row lands on the bottom row and `j`/Down from the bottom row lands back on the top row, with the normal debounce/arming rules applying to the wrapped-onto row. The debounce exists because each tmux switch costs ~80-110ms plus hook fan-out, so key-repeat browsing must not fire a switch per crossed row; only the row the highlight rests on switches. The highlighted row stays visible as the pending switch target until `YourSession`/pane identity confirms the new context; it must not snap back to the old active row for an intermediate frame. Resting back on the already-confirmed session with no switch in flight disarms the debounce and sends nothing, and re-highlighting the current in-flight switch target sends nothing, so a reversed browse (A → B → A) never emits the intermediate switch. `Enter` on a concrete session row commits immediately and supersedes the armed debounce, as do `Tab`/`Shift-Tab`, digit keys, and mouse clicks. Mouse hover stays visual-only and never switches. In all cases, the durable active row stays on the confirmed active session until confirmation.
