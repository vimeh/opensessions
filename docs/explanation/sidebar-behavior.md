...
The sidebar session list has one durable local active row: this tmux client's confirmed active session. Keyboard navigation is highlight-driven: `j`/`k`/arrow movement onto a concrete session row sends a switch intent immediately — no `Enter` required and no client-side latency. Execution is coalesced server-side by the switch gate (`SWITCH_GATE_MS`, 150ms): at most one tmux switch executes per gate window and the latest intent wins, so key-repeat scanning cannot strobe the attached client while a single press switches instantly. Client-side debouncing is deliberately absent: a switch moves keyboard focus into the destination session's sidebar process, so any per-process timer armed mid-scan outlives the user's presence in that pane and fires stale switches — coalescing must live in the one place that sees every intent, the server. Every sidebar-originated switch re-focuses the destination window's sidebar pane, so navigation keys keep landing in the sidebar until the user commits: `Enter` on a concrete session row sends the switch with `focusMain` and the server drops keyboard focus into the destination's main (non-sidebar) pane. Navigation wraps at the list edges: `k`/Up from the top row lands on the bottom row and `j`/Down from the bottom row lands back on the top row. The highlighted row stays visible as the pending switch target until `YourSession`/pane identity confirms the new context; it must not snap back to the old active row for an intermediate frame. Landing on the already-confirmed session with no switch in flight sends nothing, and re-highlighting the current in-flight switch target sends nothing. `Tab`/`Shift-Tab`, digit keys, and mouse clicks keep their existing switching behavior and stay on the sidebar; agent-row activation still focuses the target agent pane after switching. Mouse hover stays visual-only and never switches. In all cases, the durable active row stays on the confirmed active session until confirmation.

Worktree group headers are the one browse-only focus target. `j`/`k`/arrow navigation can land on them without switching anything, and `Enter` toggles collapse/expand. Once the user highlights a concrete child session inside an expanded worktree group, the switch fires and pending focus moves from the group header to that child session row; the group header must not remain focused in the destination session.

Temporary focus must not look like active focus. The confirmed active row owns the strong green active marker; a keyboard selection that has not settled yet (a group header, or a pending switch target) uses a weaker cursor marker. Otherwise browsing shows two active-looking rows and feels like focus randomly split.

Focus is allowed to diverge from the confirmed session only until a concrete activation/switch-settled event. When tmux activates session B, every sidebar pane whose own confirmed local session is B must reset its keyboard focus back to B. This is a session-scoped reset: if session B has sidebars in multiple tmux windows, all of B's sidebars reset; sidebars belonging to other sessions keep their own local focus. The reset is triggered by the settled switch/activation event, not by generic websocket state snapshots or server-owned global focus.

When a highlight, `Enter`, or `Tab` switches from session A to session B, the old A sidebar remains alive in the background. If A keeps `pending=B` forever, returning to A later replays stale focus and feels random. A server `Focus` broadcast is only a fast intent echo and can arrive before tmux visibly switches the attached client, so it must not clear pending state. A settled `State` snapshot for B is allowed to clean up this one local case: if a sidebar has `pending=B` but its own local session is still A, it clears pending and rehomes focus to A. This does not make server focus authoritative; it only cleans up the source pane after its switch request moved the attached tmux client elsewhere.
