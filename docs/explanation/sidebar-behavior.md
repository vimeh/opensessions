...
The sidebar session list has one durable local active row: this tmux client's confirmed active session. Keyboard navigation is highlight-driven: when `j`/`k`/arrow movement lands on a concrete session row, the sidebar immediately requests a switch to that session — no `Enter` required. The highlighted row stays visible as the pending switch target until `YourSession`/pane identity confirms the new context; it must not snap back to the old active row for an intermediate frame. Highlighting the already-confirmed session with no switch in flight sends nothing, and re-highlighting the current pending target sends nothing, but reversing an in-flight highlight (A → B → A) sends a new switch so the final highlighted row always wins. `Enter` on a concrete session row remains a harmless explicit activation of the same target. `Tab`/`Shift-Tab` still immediately switch to the next/previous visible session. Mouse clicks on sessions also make the clicked concrete session the pending focus target; mouse hover stays visual-only and never switches. In all cases, the durable active row stays on the confirmed active session until confirmation.
