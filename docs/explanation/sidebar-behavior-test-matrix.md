...
| --- | --- |
| Highlighting a concrete session with `j`/`k`/arrows switches after a short rest without `Enter`, the confirmed local session is not re-switched, worktree group headers stay browse-only with `Enter` toggling collapse, and group focus rehomes to the chosen child session. | `tmux_sidebar_keyboard_focus_and_worktree_flow` |
| A single `Down` switches the attached client after the highlight debounce, and returning to the origin session leaves every one of its sidebars showing the confirmed active row with no stale focus marker. | `tmux_sidebar_rehomes_focus_after_highlight_driven_switch` |
